...
import models
from utils1 import tensor2array, save_checkpoint, save_path_formatter, log_output_tensorboard, save_checkpoint_pose
...
parser.add_argument('--dataset', dest="dataset", choices=["graspnet", 'realworld'],
                    help='dataset choice')
...
def main():
    ...
    from datasets.sequence_folders import GraspNetDataset_multi, GraspNetDataset_multi_val_with_pose, \
        RealWorldDataset_multi
    ...
    print("=> fetching scenes in '{}'".format(args.dataset))
    if args.dataset == "graspnet":
        train_set = GraspNetDataset_multi(
        ...
        train_set = RealWorldDataset_multi(
            transform=train_transform,
            split='train',
            seed=args.seed,
            sequence_length=args.sequence_length
    ...
    if args.dataset == "graspnet":
        ...
        val_set = RealWorldDataset_multi(
            transform=valid_transform,
            split='test',
            seed=args.seed,
            sequence_length=args.sequence_length,
    ...
    pose_exp_net.load_state_dict(torch.load('./exp_pose_best_realworld.pth.tar')['state_dict'], strict=False)
    ...
    if args.pretrained_disp or args.evaluate:
        ...
        errors, error_names = validate_with_gt_depth(args, val_loader, None, pose_exp_net, 0, logger, tb_writer)
    ...
    for epoch in range(args.epochs):
        ...
        logger.reset_valid_bar()
        errors, error_names = validate_with_gt_depth(args, val_loader, disp_net, pose_exp_net, epoch, logger, tb_writer)
...
def train(args, train_loader, disp_net, pose_exp_net, optimizer, epoch_size, logger, tb_writer):
    ...
    for i, (img_n, img_i, intrinsics, intrinsics_inv, depth_n, depth_i, camera_pose_i, camera_pose_n) in enumerate(
            train_loader):
        ...
        depth_n = depth_n.unsqueeze(1).to(device)
        depth_n = [depth_n,
                   torch.nn.functional.interpolate(depth_n, scale_factor=0.5),
                   torch.nn.functional.interpolate(depth_n, scale_factor=0.5 * 0.5),
                   torch.nn.functional.interpolate(depth_n, scale_factor=0.5 * 0.5 * 0.5)]

        depth_i = depth_i.unsqueeze(1).to(device)
        depth_i = [depth_i,
                   torch.nn.functional.interpolate(depth_i, scale_factor=0.5),
                   torch.nn.functional.interpolate(depth_i, scale_factor=0.5 * 0.5),
                   torch.nn.functional.interpolate(depth_i, scale_factor=0.5 * 0.5 * 0.5)]
        explainability_mask, pose = pose_exp_net(img_n, [img_i])
        # explainability_mask =
        loss_1, warped, diff = photometric_reconstruction_loss(img_n, [img_i], intrinsics,
                                                               depth_n, explainability_mask, pose,
                                                               args.rotation_mode, args.padding_mode)
        loss_2 = explainability_loss(explainability_mask)
        loss = w1 * loss_1 + w2 * loss_2
        loss_3 = torch.tensor(0).cuda()
        explainability_mask, pose = pose_exp_net(img_i, [img_n])

        loss_1, warped, diff = photometric_reconstruction_loss(img_i, [img_n], intrinsics,
                                                               depth_i, explainability_mask, pose,
                                                               args.rotation_mode, args.padding_mode)
        loss_2 = explainability_loss(explainability_mask)
        loss += w1*loss_1 + w2*loss_2
...
def validate_without_gt(args, val_loader, disp_net, pose_exp_net, epoch, logger, tb_writer, sample_nb_to_log=3):
    ...
    for i, (tgt_img, ref_imgs, intrinsics, intrinsics_inv) in enumerate(val_loader):
        ...
        if log_outputs and i in batches_to_log:  # log first output of wanted batches
            ...
            log_output_tensorboard(tb_writer, 'val', index, '', epoch, 1. / disp, disp, warped[0], diff[0],
                                   explainability_mask)
...
@torch.no_grad()
def validate_with_gt_depth(args, val_loader, disp_net, pose_exp_net, epoch, logger, tb_writer, sample_nb_to_log=3):
    global device
    batch_time = AverageMeter()
    losses = AverageMeter(i=3, precision=4)
    log_outputs = sample_nb_to_log > 0
    ...
    logger.valid_bar.update(0)
    for i, (tgt_img, ref_imgs, tgt_depth, intrinsics) in enumerate(val_loader):
        tgt_img = tgt_img.to(device)
        tgt_depth = tgt_depth.to(device)
        ref_imgs = ref_imgs.to(device)
        intrinsics = intrinsics.to(device)
        depth_n = tgt_depth.unsqueeze(1).to(device)
        depth = [depth_n,
                   torch.nn.functional.interpolate(depth_n, scale_factor=0.5),
                   torch.nn.functional.interpolate(depth_n, scale_factor=0.5 * 0.5),
                   torch.nn.functional.interpolate(depth_n, scale_factor=0.5 * 0.5 * 0.5)]
...
def validate_with_gt_pose(args, val_loader, disp_net, pose_exp_net, epoch, logger, tb_writer, sample_nb_to_log=3):
    ...
    for i, (tgt_img, ref_imgs, gt_depth, gt_poses) in enumerate(val_loader):
        ...
        if log_outputs and i in batches_to_log:  # log first output of wanted batches
            ...
            log_output_tensorboard(tb_writer, 'val', index, '', epoch, None, None, None, None, explainability_mask)

        if log_outputs and i < len(val_loader) - 1:
            step = args.batch_size * (args.sequence_length - 1)
            poses_values[i * step:(i + 1) * step] = pose.cpu().view(-1, 6).numpy()
            step = args.batch_size * 3

        pose_errors.update(compute_pose_errors(gt_poses[:, :3, :].cpu(), transform_matrices))
        ...
        if i % args.print_freq == 0:
            logger.valid_writer.write(
                'valid: Time {} , ATE {:.4f} ({:.4f})'.format(batch_time,
                                                              pose_errors.val[0],
                                                              pose_errors.avg[0]))
    if log_outputs:
        prefix = 'valid poses'
    ...
    logger.valid_bar.update(len(val_loader))
    return pose_errors.avg, pose_error_names
