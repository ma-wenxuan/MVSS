...
import os
import torch
sys.path.append(os.path.join('/home/mawenxuan/mvss/lib'))
sys.path.append('/home/mawenxuan/mvss/lib/datasets')
sys.path.append('/home/mawenxuan/mvss/tools')
# sys.path.append('/home/mwx/unseenObjectClustering/utils')
from graspnet_dataset import GraspNetDataset, RealWorldDataset
...
class GraspNetDataset_multi(GraspNetDataset):
    ...
    def __getitem__(self, idx):
        ...
        depth_n = sample_n['depth_raw'] / self.factor_depth
        img_i = sample_i['image_color_raw'][:, :, [2,1,0]].float().numpy()
        depth_i = sample_i['depth_raw'] / self.factor_depth
        ...
        if self.split == 'train':
            ...
            return img_n, img_i, depth_n, intrinsics
        else:
...
class RealWorldDataset_multi(RealWorldDataset):
    def __init__(self, root='/home/mwx/AUBO_python3/saved_videos', seed=None, split='train', sequence_length=3, transform=None, target_transform=None):
        super(RealWorldDataset_multi, self).__init__(root, split=split)
        self.scene_num = len(self.sceneIds)
        self.scenes = self.sceneIds
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'train':
            ...
            return img_n, img_i, depth_n, intrinsics
            # gt_pose gt_depth
            # return img_n, img_i, depth_n, torch.linalg.inv(sample_i['camera_pose']) @ sample_n['camera_pose']
        else:
            return img_n, img_i, intrinsics, np.linalg.inv(intrinsics), depth_n, depth_i, sample_i['camera_pose'], \
                   sample_n['camera_pose']
