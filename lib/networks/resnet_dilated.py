...
class Resnet34_8s(nn.Module):
    ...
    def __init__(self, num_classes=1000, input_channels=3, pretrained=True):
        ...
        # self.pro = nn.Sequential(nn.BatchNorm2d(128),
    ...
    def forward(self, x, feature_alignment=False):
        ...
        # x = self.resnet34_8s(x)
        # x = self.pro(x)
        x = self.resnet34_8s.conv1(x)
        x = self.resnet34_8s.bn1(x)
        x0 = self.resnet34_8s.relu(x)
        x = self.resnet34_8s.maxpool(x0)

        x1 = self.resnet34_8s.layer1(x)
        x2 = self.resnet34_8s.layer2(x1)
        x3 = self.resnet34_8s.layer3(x2)
        x4 = self.resnet34_8s.layer4(x3)
        x4 = self.resnet34_8s.fc(x4)


        # x1 = nn.functional.upsample_bilinear(input=x1, size=input_spatial_dim)
        # x2 = nn.functional.upsample_bilinear(input=x2, size=input_spatial_dim)
        # x3 = nn.functional.upsample_bilinear(input=x3, size=input_spatial_dim)
        x4 = nn.functional.upsample_bilinear(input=x4, size=input_spatial_dim)
        # x = nn.functional.upsample_bilinear(input=x4, size=input_spatial_dim)
        # x = torch.concat([x4], dim=1)
        return x4
...
class Resnet50_8s(nn.Module):
    ...
    def __init__(self, num_classes=1000):
        ...
        # Randomly initialize the 1x1 Conv scoring layer
        resnet50_8s.fc = nn.Conv2d(resnet50_8s.inplanes, 512, 1)
        self.pro = nn.Sequential(nn.BatchNorm2d(512),
                                 nn.ReLU(inplace=True),
                                 nn.Conv2d(512, 64, 1))
