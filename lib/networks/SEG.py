...
from networks.embedding import EmbeddingLoss, SimCLR_Loss, Embedding_SimCLR_Loss
from . import unets
...
class SEGNET(nn.Module):
    ...
    def __init__(self, init_weights=True, batch_norm=False, in_channels=3,
        ...
        lambda_mvss = cfg.TRAIN.EMBEDDING_LAMBDA_MVSS
        lambda_dense = cfg.TRAIN.EMBEDDING_LAMBDA_DENSE
        self.embedding_loss = EmbeddingLoss(alpha, delta, lambda_intra, lambda_inter, lambda_mvss, self.metric, self.normalize)
        self.embedding_simclr_loss = Embedding_SimCLR_Loss(alpha, delta, lambda_intra, lambda_inter, lambda_mvss, lambda_dense, self.metric, self.normalize)
        ...
        if network_name == 'vgg':
            ...
            if self.input_type == 'RGBD' and self.fusion_type != 'early':
                self.fcn_depth = getattr(resnet_dilated, network_name)(num_classes=num_units, input_channels=3, pretrained=False)
    ...
    def forward(self, img, label, depth, picxy=None, selected_pixels=None, match_num=None):
        ...
        if self.training:
            ...
            # 对于加入simclr对比损失的输入，前半为正常输入，后半为对应前半的相邻帧样本。
            dense_contrastive_loss = torch.tensor(0)

            # loss, intra_cluster_loss, inter_cluster_loss = self.embedding_simclr_loss(features, label, picxy)
            # loss, intra_cluster_loss, inter_cluster_loss = self.embedding_loss(features, label)

            if picxy == None:
                loss, intra_cluster_loss, inter_cluster_loss = self.embedding_loss(features, label, )
                return loss, intra_cluster_loss, inter_cluster_loss, mvss_loss, features
            else:
                loss, intra_cluster_loss, inter_cluster_loss, mvss_loss, dense_contrastive_loss = self.embedding_simclr_loss(
                    features, label, picxy, selected_pixels, match_num, img)
                return loss, intra_cluster_loss, inter_cluster_loss, mvss_loss, dense_contrastive_loss, features
