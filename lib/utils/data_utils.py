...
def create_point_cloud_from_depth_image(depth, camera, organized=True):
    ...
    camera.cx = camera.cx.astype(np.float32)
    camera.cy = camera.cy.astype(np.float32)
    camera.fx = camera.fx.astype(np.float32)
    camera.fy = camera.fy.astype(np.float32)
    xmap = np.arange(camera.width, dtype=np.float32)
    ymap = np.arange(camera.height, dtype=np.float32)
