...
def crop_rois1(rgb, initial_masks):
    ...
    for index, mask_id in enumerate(mask_ids):
        ...
        # y_min = max(y_min - y_padding, 0)
        # y_max = min(y_max + y_padding, H - 1)
        maskt = mask[y_min:y_max+1, x_min:x_max+1]
        if torch.sum(maskt)/(maskt.size()[0]*maskt.size()[1]) > 0.3 and torch.sum(maskt)>1000:
            goodindex.append(index)
        xw = x_max - x_min
        yw = y_max - y_min
        t = max(xw, yw)
        if xw > yw:
            x_padding = int(torch.round((xw).float() * padding_percentage).item())
            y_padding = int(torch.round(((xw).float() * padding_percentage).item() + (xw - yw)/2))
        else:
            x_padding = int(torch.round(((yw).float() * padding_percentage)).item() + (yw - xw)/2)
            y_padding = int(torch.round((yw).float() * padding_percentage).item())

        # x_padding = int(torch.round((yw).float() * padding_percentage).item())
        # y_padding = int(torch.round((xw).float() * padding_percentage).item())
        ...
        rgb_crop = rgb_crop.cpu() * mask_crop
    ...
    return rgb_crops, mask_crops, rois, goodindex
...
def test_sample(sample, network, network_crop):
    ...
    if cfg.TEST.VISUALIZE:
        ...
        img = _vis_minibatch_segmentation_final(image, depth=None, label=label, out_label=out_label)
    return out_label, out_label_refined, features
...
def test_segnet(test_loader, network, output_dir, network_crop):
    ...
    for i, sample in enumerate(test_loader):
        ...
        for k in metrics.keys():
            metrics_sum[k] = metrics_sum.get(k, 0) + metrics[k]
        out = metrics_sum.copy()
        for k in out.keys():
            out[k] = out[k] / (i+1)
        print(out)
